...
export async function generateMetadata({ params }: Props) {
  ...
  if (!service) return {};
  if (service.slug === "domestic-couples") return pageMetadata({ title: "Hire a Live-In Domestic Couple in Canada | DomesticHelp.ca", description: "Hire an experienced domestic couple to care for your Canadian home or estate. Housekeeping, cooking, property oversight, driving, errands, and guest support.", path: "/services/" + service.slug });
  return pageMetadata({ title: service.title + " in Canada | Private Recruitment | DomesticHelp.ca", description: "Find a personally recruited " + service.singular + " for your Canadian household. " + service.summary + " Curated search available now; full recruitment coming soon.", path: "/services/" + service.slug });
}

export default async function ServicePage({ params }: Props) {
  const service = getService((await params).service);
  if (!service) notFound();
  const isCouple = service.slug === "domestic-couples";
